...
export default function Settings() {
  ...
  // Use Zustand store for settings data
  const { settings, setSetting, initialize, reset } = useSettingsStore();

  // Use sidebar context for navigation
  const { activeSettingsTab } = useSidebarContext();

  // Initialize store when component mounts
  useEffect(() => {
    initialize();
    return () => {
      // Reset store when component unmounts (page changes)
      reset();
    };
  }, [initialize, reset]);
  ...
  const categories = Object.keys(settingsConfig) as (keyof SettingsConfig)[];
  const allTabs = ["account" as const, ...categories];
    ...
    <Layout>
      <div className="container max-w-4xl py-8">
        {activeSettingsTab === "account" ? (
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center justify-between p-4 border rounded-lg">
                    ...
                    <Badge
                      variant={hasPasswordAccount ? "default" : "secondary"}
                    >
                  ...
                  <div className="p-4 bg-muted/50 rounded-lg">
                    <p className="text-sm text-muted-foreground">
                      <strong>Security Note:</strong> Linking accounts allows
                      you to sign in using different methods, but they must be
                      manually connected for security.
                    </p>
                  </div>
                </CardContent>
              </Card>
          ...
          settingsConfig[activeSettingsTab as keyof SettingsConfig] ? (
          <SettingsCategory
            category={activeSettingsTab as keyof SettingsConfig}
            categoryData={
              settingsConfig[activeSettingsTab as keyof SettingsConfig]
            }
            settings={settings}
